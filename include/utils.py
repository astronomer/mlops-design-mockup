...
def generate_churn_data(num_customers=1000, base_date=datetime(2024, 1, 1)):
    np.random.seed(42)
...
def generate_inference_data(num_customers=1000, base_date=datetime(2024, 1, 1)):
    np.random.seed(42)
    ...
    tenures = [(base_date - signup_date).days // 30 for signup_date in signup_dates]

    base_date_str = base_date.strftime("%Y-%m-%d")
    signup_dates_str = [date.strftime("%Y-%m-%d") for date in signup_dates]
    ...
    data = pd.DataFrame(
        {
            ...
            "product": product,
            "operating_system": operating_system,
            "churn": [0] * num_customers,
            "churn_date": [None] * num_customers,
            "last_updated": base_date_str,
        }
    )
